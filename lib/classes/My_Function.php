...
//////////////////////////////GET User id////////////////
	function getuserId($user_id)
    {
        ...
        $sql = $obj_rep->query("username", "user_registration", $condtion);
        
        $result = $obj_rep->get_all_row($sql);
        return $userId = $result['username'];
    }
	...
	function getGender($user_id)
    {
        ...
        $sql = $obj_rep->query("sex", "user_registration", $condtion);
        
        $result = $obj_rep->get_all_row($sql);
        return $gender = $result['sex'];
    }
    ...
    function matchingCommission($user_id, $userStatus)
    {
        ...
        $querys1 = mysql_fetch_array(mysql_query("select sum(mpv) as totalleftamount from manage_bv_history where status='0' and income_id='" . $user_id . "' and down_id!='".$user_id."' and leg='left'"));
        
        ////////////////////////////////////////////////////total left with carry forward//////////////    
        $leftTotal = $querys1['totalleftamount'] + $carryLeft;
		
		$newLeft=$querys1[totalleftamount];
        
        $querys12 = mysql_fetch_array(mysql_query("select sum(mpv) as totalrightamount from manage_bv_history where status='0' and income_id='" . $user_id . "' and down_id!='".$user_id."' and leg='right'"));
        
        
        ////////////////////////////////////////////////////total right with carry forward//////////////        
        $rightTotal = $querys12['totalrightamount'] + $carryReft;
		
		$newRight=$querys12[totalrightamount];
		
		$newPayble  = min($newLeft, $newRight);
		...
		//////////////////////////////////////////Matching Upgrading Centrer Commission//////////////////		
		
		$ppp= $this->personalPoint($user_id, $userStatus);
                        ...
                        else if($ppp>=100 && $ppp<=3000)
                        {
							$totPoint=$ppp*10;
                            if ($totalPair >= $totPoint) {
                            $totalPair         = $totPoint;
                            $leftCarryForward  = 0;
                            $rightCarryForward = 0;
        ...
        if ($totalPair >= 100) {
		    ...
		    if (mysql_num_rows($sqlmathicng) > 0 && $userStatus==3) {
                ...
                mysql_query("update final_e_wallet set amount=(amount+$fetDatas[commission]) where user_id='" . $user_id . "'");
							...
							$whmat = " user_id='" . $user_id . "' and status='0'";
							$obj_rep->update_tbl($matsts, 'matching_income', $whmat);
    ...
    function updateMatchingComm($user_id,$curppp)
    {
        global $obj_rep;
        $sqlmathicng = mysql_query("select * from matching_income where user_id = '" . $user_id . "' and status='0'");
		
		$sqlsts=mysql_fetch_assoc(mysql_query("select * from user_registration where user_id='".$user_id."'"));
