...
class My_Function
{
	...
	$sqlLevlcheck = mysql_query("select * from level_income where income_id='" . $user_id . "' and status='0'");
        ...
        $selectComission = mysql_fetch_array(mysql_query("SELECT * FROM set_comission"));
        $levcom          = explode(',', $selectComission['level_comission']);
        
        if (mysql_num_rows($sqlLevlcheck) > 0) {
            while ($fetchLevel = mysql_fetch_assoc($sqlLevlcheck)) {
                $sqlCommi = mysql_fetch_assoc(mysql_query("select * from purchase_detail where invoice_no='" . $fetchLevel['invoice_no'] . "'"));
                
                if ($fetchLevel['level'] == 1) {
                    $commiss = $levcom[0];
                } else if ($fetchLevel['level'] == 2) {
                    $commiss = $levcom[1];
                }
                
                else if ($fetchLevel['level'] == 3) {
                    $commiss = $levcom[2];
                }
                
                else if ($fetchLevel['level'] == 4) {
                    $commiss = $levcom[3];
                }
                
                else if ($fetchLevel['level'] == 5) {
                    $commiss = $levcom[4];
                }
                
                else if ($fetchLevel['level'] == 6) {
                    $commiss = $levcom[5];
                }
                
                else if ($fetchLevel['level'] == 7) {
                    $commiss = $levcom[6];
                }
                
                
				$commission = ($sqlCommi['product_volume'] * ($commiss / 100)) * $selectComission['pntamount'];
