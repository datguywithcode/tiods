...
$this->mysql_func->query_execute("insert into subscription set order_no='Admin',user_id='$row_u[user_id]',subs_fee='160',payment_mode='From Admin',subs_date='$bonus_date',end_date='$expire_date'");
	...
	{	
	 ...
	 $table_name=TABLE_PREFIX.'pins';
		...
		{
		header("Location: http://localhost/greenwealth/admin/admin_main.php?page_number=190");
		}
		else
		{
		
		header('Location: http://' . $_SERVER['HTTP_HOST'] . '/~sandeep/greenwealth/admin/admin_main.php?page_number=190');
	...
	{	
		...
		{
			$this->mysql_func->update_tbl($update_arr,$table_name,$where);
			$this->mysql_func->insert_tbl($update_arr,$table_history);
		}
		else
		{	
		  //echo "<pre>"; print_r($update_arr);exit;
...
return $price;
	...
	function _Completed_Withdraw_Bank()
	{
		...
		foreach($_POST['id'] as $key=>$val)
		{
			$update_arr=array("status"=>2,"response"=>$_POST['response'],"varify_date"=>$curdate);
			$where=" id='$val'";
		...
		$update=2;
		$error=0;
		$msg="Bank Transfer Request Completed.";
		header("Location:admin_main.php?page_number=44&update=".$update."error=".$error."&msg=".$msg);
	}
	...
	function BV_Price()
{
	...
	function _Payment_Bank()
	{
		...
		header("Location:admin_main.php?page_number=247");
	}
